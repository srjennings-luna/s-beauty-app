...
| `content-docs/KALLOS-CC-Dashboard-Enhancement-Brief.html` | **In repo** — Claude Code dashboard enhancements brief. TTS columns, filter/sort, photography + ancient era schema additions, reflectionQuestionsAudio field, Sanity Presentation plugin (stretch). Run after schema design brief is complete. Implements R1 (artworkHook rename) and R5 (journeyDay promotion), adds genre + split audio source fields, fixes R6 and two rendering bugs, updates dashboard. Run AFTER reading the audit brief. Opus 4.6 required. READ THIS before any architecture or schema work. Three tasks: schema audit, full content inventory (including Go Deeper tracker), and regenerable content dashboard. No code changes during the audit session — query and report only. |
...
| `contentItem` | ✅ New — `curatorNote` renamed to `artworkHook` (April 24, 2026 — R1). Artwork-level hook, safe to display anywhere the piece appears. 41 existing records migrated; 6 flagged REVIEW still hold text under legacy `curatorNote` until rewritten. Auditio fields shared with dailyPrompt / journeyDay. |
...
| `journeyDay` | ✅ Standalone document (April 24, 2026 — R5 migration complete). 25 docs live. Each has a required `journey` back-reference. Fields: dayNumber, dayTitle, openImage, openText(+audio), encounterContent (ref → contentItem), encounterGuidance, encounterNote(+audio), auditio (now with composerArtist/workTitle/genre), lectio, reflectionQuestions (renamed from reflectQuestions, April 24), connectThread, goDeeper. |

...
6. `content-docs/CONTUERI-PreDraft-Checklist.html`, the pre-draft checklist (added June 22, 2026 from a Cowork editorial rewrite session). The two rules at the top (angle menu before prose, then story not lecture) prevent most rework. The 10-item self-score below is what every draft passes before sending. Includes the Verba rule (Latin audio requires Latin + English translation for the entire composition; English audio lyrics are optional).

**Then read the voice-calibration snapshot** before drafting any content or editorial work:

7. `content-docs/CONTUERI-Voice-Calibration.html`, the snapshot of the two canonical voice exemplars (the When Myth Became Fact journey and the "They Were Early" Justin Martyr Pause & Ponder), pulled from live Sanity. The four foundational docs above abstract the voice in rules. This file carries the voice itself. A session that drafts content without reading it will produce doctrine-compliant prose in the wrong voice. This was tested June 18, 2026: a Cowork session writing a color iconography P&P drifted toward a glossary/decoder-tour shape until it was sent to read the live published pieces; reading them fixed more than any abstract description of the voice could.
...
| `CONTUERI-ContentItem-Checklist.html` | **Cowork doc (Documents folder)** — per-type checklist for all 9 contentItem types (sacred-art, photography, thinker, literature, music, food-wine, landscape, watch-listen, math-science) plus Daily Prompt fields. REQUIRED fields highlighted in gold; type-specific required fields in orange. Sanity field names in green monospace throughout. Includes VD override fields. Created June 7-8, 2026. |
...
### Phase 2 Work Done (June 7-8, 2026 · Cowork content infrastructure + ContentItem checklist + CC data-gaps brief + session benchmark)

Cowork session running parallel to the CC TR seed. Completed Task #22 and established content quality tracking infrastructure.

- ✅ **CONTUERI-ContentItem-Checklist.html built** (`~/Documents/`). Per-type checklist for all 9 contentItem types (sacred-art, photography, thinker, literature, music, food-wine, landscape, watch-listen, math-science) plus Daily Prompt fields. REQUIRED fields highlighted. Sanity field names in green monospace. Includes VD override fields (customPrayerPrompt, customActioHeadline, customActioInstruction). Reference doc for any future Cowork content session entering data into Sanity.
- ✅ **CONTUERI-CC-DataGaps-Brief.html built** (`~/Documents/`). Post-seed action brief identifying fields missing or empty in the 30-day P&P seed after Sheri reviewed in Studio. Section 1: fields to verify (potential silent drops). Section 2: missing required fields. Section 3: missing optional fields. Section 4: location data table (all 30 days). Section 5: themes as `_refs` not strings. Section 6: audio deferred/included question. 7 actionable questions for CC. CC patch script (`b2f1515f`) resolved the description/location/dayTitle/workTitle/mediaUrl gaps for the 20 seeded days.
- ✅ **KALLOS-Session-Benchmark.html created** (`~/Documents/`). New content quality KPI tracking doc. Baseline established from Apr 14-15 BTG session (Claude 21%, Sheri 79%). First scored session: Jun 7-8 (Claude 48%, Sheri 52%), +27 percentage points from baseline. Key insight: Claude catches structural/schema errors reliably but misses actio specificity and editorial voice (actios were 100% Sheri-caught). Actio specificity rule added to Voice Rules from this finding.
- ✅ **CONTUERI-PP-30Day-Content-Plan-v3.html updated** (`~/Documents/`). All 10 skipped days (6, 12, 15, 16, 19, 22, 24, 26, 27, 29) now marked with Skipped badge and auto-link note. v3 plan confirmed as system of record for all 30 P&P days.

### Phase 2 Work Done (June 9, 2026 · Explore Cards build + Visio Divina meditative typography pass)

Full implementation of the June 9 Explore Cards Build Brief (`content-docs/CONTUERI-Explore-Cards-Build-Brief.html`), shipped as five phases A → B → C → C.1 → D plus an architecture fix and a CTA color/weight decision Sheri overrode from the brief. Followed by a Meditate-step typography pass that locked the contemplative-app weight pattern across all VD section headings. Plus an URGENT GROQ fix that was a hard blocker for the June 12 launch.

**URGENT: `getDailyPrompt` clamp to past dates** (commit `fc44d8ea`). After the 30-day P&P seed landed prompts dated June 12 through July 11, the fallback `*[_type == "dailyPrompt" && defined(date)] | order(date desc)[0]` started returning the LATEST seeded prompt (July 11) on every gap day — showing users content scheduled weeks in the future. Fix: add `date <= $today` clause + `{today: targetDate}` param. Preview path keeps unbounded fallback so Studio Presentation can still preview future drafts. One-line GROQ patch, shipped first.

**Phase A — Explore data layer** (commit `e344e576`). `EXPLORE_ATTRIBUTION_GROQ` per-content-type `select()` produces a single human-readable attribution string for each of the 9 schema-valid contentTypes — "Rembrandt van Rijn, c. 1669" for sacred-art, "Thomas Aquinas · medieval" for thinker, etc. Defensive against missing sub-fields. Falls through to the legacy `attribution` field for orphan records. `EXPLORE_DETAIL_FIELDS` projection bundles contentItem fields + computed attribution + reverse-ref lookups (`journeys`, `dailyPrompts`) for contextual links. `getExploreDetailItem(id)` single-fetch helper. New `ExploreDetailItem` type. Library URL handler for the Explore detail card's P&P contextual link: `PromptCard` exports `anchorIdFromDate(date)` and tags each card with `id="june-12"` style. Library page reads `?expand=june-2026` on mount, derives the matching monthKey (`2026-06`), and removes it from `collapsedMonths` if present (defensive — today's default is all-expanded). After async content loads, a double-rAF effect programmatically scrolls to `window.location.hash` via `scrollIntoView({behavior: 'smooth'})`. Next.js does NOT auto-scroll to hashes once content lazy-loads via useEffect, so we do it ourselves.

**Phase B — ExploreDetailCard for sacred-art + landscape** (commit `186a9aa7`). New `components/ExploreDetailCard.tsx`. Sacred-art + landscape contentItems used to bypass any intermediate surface and tap straight into Visio Divina (June 5 call). They now stop on a detail card first: image + type label in P&P gradient color + title + attribution + description + ENTER VISIO DIVINA → CTA + contextual journey + P&P links. Espresso bg, cream-tone text. Display labels added to `lib/contentTypeColors.ts` as `CONTENT_TYPE_DISPLAY_LABELS` (9-type map for the uppercase brand-consistent type label).

**Phase C — All 9 types route through ExploreDetailCard; REFLECT for non-VD types** (commit `bbed38f0`). Sacred-art + landscape keep ENTER VISIO DIVINA; other 7 (photography, thinker, literature, music, food-wine, watch-listen, math-science) get REFLECT (text + chevron, no border box) below the description. "PRAY WITH THIS IMAGE" retired everywhere. REFLECT toggles an inline expand panel with Scripture / Quote / Reflection Questions (the same content surfaces ArtworkViewer used to render). Map onMarkerClick also routes through ExploreDetailCard. Legacy `<ArtworkViewer>` render block removed from Explore (ArtworkViewer kept for Library). `useRouter` import dropped.

**C.1 — Cleanup pass after Sheri's testing** (commit `75c41bd8`). Four real fixes surfaced during verification:
1. **Image cropping bug.** ExploreDetailCard used `object-cover` with `objectPosition: "center bottom"` — fills container by cropping. Tall portraits (Thomas More, Hildegard) had tops lopped off. First fix: `object-cover` → `object-contain`.
2. **Context bleed on Explore.** Brief: NO context on Explore. I had projected `contentItem.context` and rendered it inside REFLECT. Removed from `EXPLORE_DETAIL_FIELDS`, from `ExploreDetailItem` type, and from the render. Scripture / Quote / Reflection Questions still appear in REFLECT.
3. **List card type label color.** Theme content list cards used sage `#7a9a8a` for the type label. Brief intent: type label is a single-color signal across both list and detail card — Mineral Blue for sacred-art, Sage Stone for landscape, etc. Switched to `getContentTypeColor()`.
4. **List card label text parity.** Two parallel maps existed — `CONTENT_TYPE_LABELS` (local, plural — "Thinkers") and `CONTENT_TYPE_DISPLAY_LABELS` (lib, singular — "THINKER"). List showed THINKERS while detail card showed THINKER for the same contentType. Retired the local map; both surfaces now read from `getContentTypeLabel()`.

**Image architecture pivot — match VD Gaze exactly** (commits `969308bb`, `c5622ebd`, `1bad9977`). The `object-contain` fix from C.1 traded cropping for letterbox bars on every aspect ratio mismatch. Two more rounds before locking the answer:
- First retry: fixed 65dvh height + `object-cover` (no bars but huge gap above image because viewport had more height available than 65dvh).
- Final answer: image area = `flex-1` (absorbs all remaining vertical space) + `object-cover` (no bars) + chrome absolutely positioned at top:0 over the image with a subtle dark gradient backdrop and drop-shadow on the close icon. Image now reaches y=0 (top of viewport) just like VD Gaze. The flex-1 + object-cover combination was the missing answer all along — the earlier flex-1 + object-contain combo created bars, fixed 65dvh + object-cover created the gap, only flex-1 + object-cover gives full-bleed in every direction.

**ENTER VISIO DIVINA — type color, not gold; font-medium, not semibold** (commit `1fc4e6ff`). Cowork's brief had ENTER VISIO DIVINA in gold. Sheri's call: violated the brief's "type label is THE ONLY colored text" rule and read as SaaS marketing rather than contemplative. Switched to typeColor + font-medium. Same color as the SACRED ART / LANDSCAPE label above so the card reads as one continuous color thread. Single-accent-per-screen is the Calm / Headspace / Endel pattern, not the SaaS gold-CTA pattern. Color-matching effect: when label + CTA share color, the eye groups them as a pair; weight asymmetry then creates "color theme + functional hierarchy."

**Phase D — content-type label on VD chrome, then reverted** (commits `d86d1a7e` → `e3494e61`). Initial implementation added SACRED ART (or other contentType) label below the GAZE/MEDITATE/etc. step name in the chrome row, colored in P&P gradient color, per the brief's "type label on VD screens" line. Sheri's call after testing on El Greco Gethsemane: the persistent label visually conflicts with the progress-bar segments at the top of CONTEMPLATE (and likely other steps), and more importantly the editorial intent is "Explore-detail-card-only" — once the user crosses the threshold into VD, the artwork is the focal point and any persistent type label competes for attention. The single-accent rule still applies; in VD the count is **zero**, not one. Reverted to step-name-only chrome.

**Meditate-step typography pass — meditative-register weights** (commits `bcfdbb18` → `f68dc6dc` → `eb631d39`). Three section-heading weights on the Meditate step (Reflect, Go deeper, Hospitality is prayer card title) read as content-marketing semibold. Two rounds of weight drops + size bump + gold border thin:
- TR short quote gold left border: `border-l-2` (2px) → `border-l` (1px) inline style. The 1px gold thread reads as a quiet line rather than a hard rule. **Pray-step scripture/quote blocks still at 2px** — separately scoped; not pointed at this pass. (Visual rule still locked to: gold left border on quoted material = 1px wherever the contemplative register dominates.)
- TR short quote size: `text-sm` (14px) → `fontSize: '1.3rem'` (~21px). Brief rule: Cormorant Garamond minimum 1.3rem. text-sm violated this; the quote was reading smaller than the body paragraph below it, inverting the hierarchy. 1.3rem + line-height 1.4 gives the quoted material the meditative presence it deserves.
- "Reflect" h2: `font-semibold` → `font-normal` → `font-light` (one rung per round).
- "Go deeper" label: `font-semibold` → `font-medium` → `font-normal`.
- TR card title (e.g., "Hospitality is prayer"): `font-semibold` → `font-medium` → `font-normal`.

**Contemplative-app typography pattern locked from this session.** "Artwork shouts, words breathe." Section headings on espresso surfaces should NOT use font-semibold or font-bold. Locked weight range:
- Single short word section heading (e.g., "Reflect" h2, large size): font-light (300)
- Section grouping label or card title (e.g., "Go deeper", TR card title "Hospitality is prayer"): font-normal (400)
- Body / instructional prompts: font-normal (400)
- Quoted material (Cormorant Garamond italic): minimum 1.3rem fontSize per brief; weight inherits
- Attribution / source lines: regular weight, color dim
...
- ✅ **Migrate GlobalMap.tsx to contentItem + 9-color content-type palette** (Manual Task #119 — CC — RESOLVED June 18, 2026). The map was already pulling from contentItem records (mappable = contentItems with coordinates), but `toArtwork` in explore/page.tsx and library/page.tsx was lossily collapsing 9 contentType values into 5 legacy locationType buckets via `CONTENT_TYPE_TO_LOCATION`, and GlobalMap.tsx was then mapping those 5 buckets to a hardcoded 5-color palette (maroon, plum, gold, rust, green) that wasn't even the canonical CONTUERI palette. Fix shipped: added `contentType` field to the `Artwork` type, passed it through in both `toArtwork` adapters (alongside the existing locationType which PrayClient + ArtworkViewer still use for sacred-art-routing checks), updated GlobalMap.tsx to use `artwork.contentType` and `getContentTypeColor()` from `lib/contentTypeColors.ts`. Markers now use the canonical 9-color palette (Mineral Blue for sacred-art, Old Ochre for literature, etc.). Visual signature is consistent across the P&P gradient, Explore type labels, and the map. PM Brief warning callout removed.
...
- **Weight rule on espresso (contemplative-app pattern, locked June 9, 2026):** section headings should NOT use font-semibold or font-bold. Single-short-word section heading (e.g., "Reflect" h2): font-light (300). Section grouping label or card title (e.g., "Go deeper", TR card title): font-normal (400). Body / instructional: font-normal (400). Attribution: regular, color dim. Hierarchy lives in size + color (`creamWarm` / `cream` / `creamDim`) + font family (italic serif vs sans), NOT weight. font-light is the floor on dark espresso; font-thin (200) starts to vibrate. Rule applies to VD screens AND any future contemplative surface. Existing Journey step typography should be audited; some headings may still be at font-semibold per the older pattern.
...
### Manual tasks added June 8, 2026 (P&P date query fix)

113. **Fix `getDailyPrompt()` fallback query (CC).** When no prompt exists for today's date, the app currently returns the highest-dated prompt in the dataset (July 11 Day 30) because the fallback uses `order(date desc)[0]` without a date ceiling. Fix: change the fallback to `date <= $today | order(date desc)[0]` so it returns the most recent past prompt instead of a future one. Confirmed behavior: seeding all 30 days at once via script (not manual daily publish) exposes this bug immediately since all future dates are live. Must be fixed before June 12 (Day 1 goes live).

### Manual tasks added June 7-8, 2026 (30-day P&P Cowork follow-up)

111. **Source and write 10 replacement content items (Cowork).** Days 6, 12, 15, 16, 19, 22, 24, 26, 27, 29 are flagged as skipped in `CONTUERI-PP-30Day-Content-Plan-v3.html`. Current placeholder items were either copyright-blocked (Salgado Day 6, Adams Day 16, Cartier-Bresson Day 26, Warner Bros Day 29) or flagged `needs_cowork` / `marginal-review` (Days 12, 15, 19, 22, 24, 27). Cowork task: find replacement content for each day, draft the seeding JSON or entry guide, and notify CC to run `npx tsx scripts/seed-pp-content.ts --patch` then `npx tsx scripts/seed-tr-content.ts --patch` to auto-link TRs. Reference v3 plan for content type and arc position per day.

112. **Create audio auditio documents in Sanity for Days 3, 15, and 22 (Sheri manual).** These days have audio suggestions in the v3 plan but the `auditio` documents were not created during the seed run. Auditio is a separate Sanity document type (standalone music content for the Explore page, not inline on contentItem). Steps: Sanity Studio → "Auditio" → New document → fill title, artist, mediaUrl, mediaType, themes. Then link the auditio doc to the corresponding `dailyPrompt.auditio` reference field for that day.
